...
unsetopt APPEND_HISTORY       # NO -> If this is set, zsh sessions will append their history list to the history file, rather than overwrite it.
...
unsetopt BRACE_CCL              # Expand expressions in braces which would not otherwise undergo brace expansion to a lexically ordered list of all the characters.
...
setopt CASE_MATCH             # Make regular expressions using the zsh/regex module (including matches with =~) sensitive to case.
unsetopt CDABLE_VARS          # NO -> If the argument to a cd command (or an implied cd with the AUTO_CD option set) is not a directory, and does not begin with a slash, try to expand the expression as if it were preceded by a '~'
...
unsetopt CORRECT              # NO -> Try to correct the spelling of commands.
...
unsetopt CSH_JUNKIE_LOOPS       # Allow loop bodies to take the form `list; end' instead of `do list; done'.
...
unsetopt DVORAK               # NO -> Use the Dvorak keyboard instead of the standard qwerty keyboard as a basis for examining spelling mistakes for the CORRECT and CORRECT_ALL options and the spell-word editor command.
setopt EQUALS                 # Perform = filename expansion. (See the section 'Filename Expansion'.)
unsetopt EMACS                # NO -> Equivalent effect of ‘bindkey -e’.
setopt EXTENDED_GLOB          # Treat the `#', `~' and `^' characters as part of patterns for filename generation, etc.
setopt EXTENDED_HISTORY       # Save each command's beginning timestamp (in seconds since the epoch) and the duration (in seconds) to the history file.
unsetopt FORCE_FLOAT          # NO -> Constants in arithmetic evaluation will be treated as floating point even without the use of a decimal point.
...
unsetopt GLOB_STAR_SHORT        # NO -> The pattern ‘**/*’ can be abbreviated to ‘**’ and the pattern ‘***/*’ can be abbreviated to ***.
...
unsetopt HASH_LIST_ALL        # NO -> Whenever a command completion is attempted, make sure the entire command path is hashed first. This makes the first completion slower.
unsetopt HIST_BEEP            # NO -> Beep when an attempt is made to access a history entry which isn't there.
unsetopt HIST_EXPIRE_DUPS_FIRST # NO -> If the internal history needs to be trimmed to add the current command line, setting this option will cause the oldest history event that has a duplicate to be lost before losing a unique event from the list.
...
unsetopt HIST_LEX_WORDS       # NO -> When this option is set, words read in from a history file are divided up in a similar fashion to normal shell command line handling.
...
unsetopt MARK_DIRS            # Append a trailing / to all directory names resulting from filename generation (globbing).
...
setopt PIPE_FAIL            # If this option is set, the exit status instead reflects the status of the rightmost element of the pipeline that was non-zero, or zero if all elements exited with zero status.
unsetopt POSIX_BUILTINS     # NO -> When this option is set the command builtin can be used to execute shell builtin commands. Parameter assignments specified before shell functions and special builtins are kept after the command completes unless the special builtin is prefixed with the command builtin. Special builtins are ., :, break, continue, declare, eval, exit, export, integer, local, readonly, return, set, shift, source, times, trap and unset.
unsetopt POSIX_CD           # NO -> Modifies the behaviour of cd, chdir and pushd commands to make them more compatible with the POSIX standard.
unsetopt POSIX_JOBS         # NO -> This option makes job control more compliant with the POSIX standard.
...
unsetopt RC_EXPAND_PARAM      # Array expansions of the form foo${xx}bar, where the parameter xx is set to (a b c), are substituted with fooabar foobbar foocbar instead of the default fooa b cbar.
...
unsetopt SH_NULLCMD         # NO -> Do not use the values of NULLCMD and READNULLCMD when doing redirections, use ‘:’ instead (see Redirection).
...
unsetopt VI                 # NO -> Equivalent effect of ‘bindkey -v’.
